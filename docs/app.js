...
extractBtn.addEventListener('click', async () => {
    ...
    try {
        const events = await extractEventsFromText(text, apiKey);
...
async function extractEventsFromText(text, apiKey) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`;
    
    const requestBody = {
        contents: [{
            parts: [{
                text: `Extract all future events from the following text. For each event, identify the title/summary, date and time, and location if available. Return ONLY a JSON array with no markdown formatting.\n\nText:\n${text}`
            }]
        }],
        generationConfig: {
    ...
    if (!response.ok) {
        ...
        throw new Error(errorData.error?.message || 'Failed to extract events');
    }
    ...
    const events = JSON.parse(textContent);
